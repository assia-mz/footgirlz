...
export const cleanValue = (value, fallback = 'TBD') => {
    ...
    if (typeof value === 'object') return value.title || value.shortName || fallback;
    return fallback;
};
...
export const getMatchValue = (item, paths, fallback = 'TBD') =>
    cleanValue(
        paths
            .map((path) => path.split('.').reduce((data, key) => (data ? data[key] : undefined), item))
...
export const getImageValue = (item, paths) => {
    const value = paths
        .map((path) => path.split('.').reduce((data, key) => (data ? data[key] : undefined), item))
        .find((candidate) => typeof candidate === 'string' && (candidate.startsWith('http') || candidate.startsWith('//')));
...
export const loadFootballDashboard = async () => {
    ...
    settled.forEach((result, index) => {
        ...
        if (result.status === 'fulfilled') {
            data[key] = findArray(result.value?.data);
        }
    });
